...
### 3. 无线通信
- 无线通信模块主要用于与外部设备进行数据交换。在当前的程序中，使用串口向小车发送任何内容都可以使得小车停下，避免撞到什么。再次发送效果约等于按下RST，但是不会有初始化的过程。

### 4. 传感器数据处理
- 传感器数据处理模块主要用于读取和处理传感器数据。主要函数包括`Line_read_raw()`和`Line_calculate_position()`。`Line_read_raw()`函数用于读取原始传感器数据，`Line_calculate_position()`函数用于计算线的位置。`Position`从-100到100，其中0表示线在中间，-100表示线在最左边，100表示线在最右边。`Line_calculate_position()`函数还有环岛标志检测的功能。
...
## 算法类
### 1. PID控制
- PID控制算法用于调整电机和舵机的控制参数，以实现精确的控制。主要函数包括`PID_control_straint()`和`PID_control_ring()`。`PID_control_straint()`函数用于循线行驶的PID控制，`PID_control_ring()`函数用于环岛行驶的PID控制。这两个函数的实现并没有什么不同，后者只是为了防止一些不必要的递归调用问题。参数也是通用的。不过需要注意的是，这两个函数都是计算当前状态下的控制量，将舵机设置到一个合适的角度。你将需要在一个循环中不断地以一定周期调用这些函数。（你当然可以把`PID_control_ring`删掉，小车还能正常运行，只是编译器会出现一个新的warning）
...
- 具体而言，在触发入环信号时，由于触发的位置一般来说比较固定，所以我们可以反复调试出最佳的入环位置。这一步通过编码器实现，到预定位置后，我们把舵机打到一个固定角度，并保持一段距离或时间（当前代码中我们是保持一段距离），再把控制权交给PID控制。PID控制会使车辆在环岛中行驶，直到环岛结束，车会在PID的控制下自动地出环。

## 项目结构
- `Libraries`目录包含了项目所需的各种库文件，用的逐飞库。
- `Project/CODE`目录包含了项目的核心代码，包括硬件驱动和算法类。
- `Project/USER`目录包含了用户自定义的代码，其实也就主函数和中断。目前暂时没有用中断。
- `Project/MDK`目录包含了MDK工程文件，不用管。
